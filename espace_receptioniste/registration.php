...
if($full){
    ...
    if ($pass==$cpass) {
        ...
        $req="UPDATE `recep` SET `age`=$age,`sexe`='$sexe',`adresse`='$addresse',`tel`=$tel,`login`='$uname',`pass`='$pass',`photo`='$chemin' WHERE `matricule`='$matricule'";
        $result=mysqli_query($con,$req);

        if($result){
            header("location: connectR.php");
        }
